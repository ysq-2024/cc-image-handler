...
插件支持 Linux、macOS 和 Windows，启动时自动检测平台并适配行为：

- **路径规范化**：所有文件路径经 `os.path.normpath()` 处理，消除 `..` 和 `.` 冗余组件，防止缓存未命中（如 `/mnt/workspace/../test.jpg` → `/mnt/test.jpg`）
- **缓存 key 规范化**：在 Windows（大小写不敏感文件系统）上，缓存 key 统一小写，`C:\Users\Test\photo.png` 和 `c:\users\test\photo.png` 共用同一条缓存。POSIX 上保留原始大小写。
- **路径检测**：用户提示中的路径同时识别 `/` 和 `\` 分隔符。Windows 匹配盘符路径（`C:\...`、`D:/...`），POSIX 匹配 Unix 路径（`/home/...`、`~/...`）。
- **信号处理**：仅在 POSIX 上注册 SIGTERM 处理器；Windows 上跳过，避免崩溃。
- **环境变量**：Windows 上 `PWD` 通常不存在，使用 `CD` 环境变量作为回退。
- **Cairo 安装**（SVG 支持）：
  - Ubuntu/Debian：`sudo apt install libcairo2-dev`
  - macOS：`brew install cairo`
  - Alpine：`apk add cairo-dev`
  - Windows：安装 GTK 运行时（详见 [cairosvg 文档](https://cairosvg.org/)）